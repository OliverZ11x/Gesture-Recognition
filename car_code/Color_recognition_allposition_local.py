    ...
    while True:
        ...
        if len(cnts_red) > 0:
            ...
            if radius1 > 15:
                ...
                # 画出重心
                cv.circle(frame, center_red, 5, (255, 0, 0), -1)

        if len(cnts_blue) > 0:
            # 找到面积最大的轮廓
            c_blue = max(cnts_blue, key=cv.contourArea)
            # 使用最小外接圆圈出面积最大的轮廓
            ((x2, y2), radius2) = cv.minEnclosingCircle(c_blue)
            # 计算轮廓的矩
            ...
            # 只处理尺寸足够大的轮廓
            if radius2 > 15:
                cv.circle(frame, (int(x2), int(y2)), int(radius2), (0, 0, 255), 2)
                cv.circle(frame, center_blue, 5, (0, 0, 255), -1)

        if len(cnts_green) > 0:
            # 找到面积最大的轮廓
            c_green = max(cnts_green, key=cv.contourArea)
            # 使用最小外接圆圈出面积最大的轮廓
            ((x3, y3), radius3) = cv.minEnclosingCircle(c_green)
            # 计算轮廓的矩
            ...
            if radius3 > 15:
                # 画出最小外接圆
                cv.circle(frame, (int(x3), int(y3)), int(radius3), (0, 255, 0), 2)
